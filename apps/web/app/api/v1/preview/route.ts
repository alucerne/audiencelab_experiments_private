...
export async function POST(request: NextRequest) {
  try {
    ...
    const loaderSQL = audience.format === 'parquet'
      ? `DROP TABLE IF EXISTS v1_preview_current; CREATE TABLE v1_preview_current AS SELECT * FROM read_parquet('${audience.url}');`
      : `DROP TABLE IF EXISTS v1_preview_current; CREATE TABLE v1_preview_current AS SELECT * FROM read_csv_auto('${audience.url}', header=true);`;
    ...
    return NextResponse.json(
      { error: `V1 preview failed: ${error.message}` },
      { status: 500 }
    );
  }
} 