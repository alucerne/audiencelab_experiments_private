...
export async function POST(request: NextRequest) {
  try {
    ...
    const loaderSQL = audience.format === 'parquet'
      ? `DROP TABLE IF EXISTS preview_current; CREATE TABLE preview_current AS SELECT * FROM read_parquet('${audience.url}');`
      : `DROP TABLE IF EXISTS preview_current; CREATE TABLE preview_current AS SELECT * FROM read_csv_auto('${audience.url}', header=true);`;
