...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      fields,
      message: `Successfully loaded ${rows.length} rows from CSV`
    });

  } catch (error: any) {
    console.error('Pixel fetch-live error:', error);
    return NextResponse.json(
      { error: `Pixel fetch-live failed: ${error.message}` },
      { status: 500 }
    );
  }
} 