...
export async function enrichRow(
  ...
  try {
    ...
    // Add available identifiers from the row - be more flexible with field names
    ...
    // Check if we have at least one identifier - be more flexible
    const hasEmail = request.email && request.email.trim() !== '';
    const hasDomain = request.domain && request.domain.trim() !== '';
    const hasCompanyName = request.company_name && request.company_name.trim() !== '';
    
    if (!hasEmail && !hasDomain && !hasCompanyName) {
      console.warn('No identifiers available for enrichment:', {
        row: Object.keys(row),
      ...
      return {
        error: 'No identifiers (email, domain, or company_name) available for enrichment',
      };
    }

    console.log('Enrichment request with identifiers:', {
      email: request.email,
    ...
    // Primary: Call the Edge Function via server-side proxy (avoids CORS issues)
    try {
      console.log('🎯 Using Supabase Edge Function via server-side proxy (primary method)');
      ...
      console.log('✅ Edge Function enrichment successful (via test endpoint):', responseData);
      
      // Extract the actual enrichment data from the test response
      const data = responseData.data || responseData;
      return data as EnrichmentResponse;

    } catch (proxyError) {
      console.error('❌ Edge Function proxy failed:', proxyError);
      
      // Fallback 1: Try local API if Edge Function fails
      try {
        console.log('🔄 Fallback 1: Trying local API endpoint');
        ...
        const response = await fetch('/api/enrich', {
          ...
          body: JSON.stringify(request),
          // Add timeout to prevent hanging requests
          signal: AbortSignal.timeout(15000), // 15 second timeout for local API
        });
        ...
        console.log('✅ Local API enrichment successful:', data);
        return data as EnrichmentResponse;

      } catch (localApiError) {
        console.error('❌ Local API also failed:', localApiError);
        
        // Fallback 2: Use mock data as final fallback
        console.log('🔄 Fallback 2: Using mock enrichment data due to all API failures');
        return generateMockEnrichment(request);
      }
    }
  } catch (error) {
    console.error('Enrichment error:', error);
    
    // If there's a network error or other issue, provide mock data
    console.log('Using mock enrichment data due to network error');
    const request: EnrichmentRequest = {
      enrich: fieldsToEnrich,
    };
    if (row.business_email) request.email = row.business_email;
    if (row.email) request.email = request.email || row.email;
    if (row.personal_email) request.email = request.email || row.personal_email;
    if (row.domain) request.domain = row.domain;
    if (row.company_domain) request.domain = request.domain || row.company_domain;
    if (row.company_name) request.company_name = row.company_name;
...
export async function enrichBatch(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const enrichment = await enrichRow(row, fieldsToEnrich);
    
    // Check if enrichment was successful
    if (enrichment.error) {
      errorCount++;
      console.warn(`Enrichment failed for row ${i + 1}:`, enrichment.error);
    } else {
      successCount++;
    }
  ...
  // Log summary
  console.log(`Enrichment batch completed: ${successCount} successful, ${errorCount} failed out of ${rows.length} total`);
