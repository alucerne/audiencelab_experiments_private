...
VALUES (makerkit.get_account_id_by_slug('audiencelab'), 'stripe', 'cus_test');

-- Call the upsert_subscription function
SELECT public.upsert_subscription(makerkit.get_account_id_by_slug('audiencelab'), 'cus_test', 'sub_test', true, 'active', 'stripe', false, 'usd', now(), now() + interval '1 month', '[
    {
        "id": "sub_123",
...
SELECT public.upsert_subscription(makerkit.get_account_id_by_slug('audiencelab'), 'cus_test', 'sub_test', false, 'past_due', 'stripe', true, 'usd', now(), now() + interval '1 month', '[
    {
        "id": "sub_123",
        "product_id": "prod_test",
        "variant_id": "var_test",
...
select is(
    (public.has_active_subscription(makerkit.get_account_id_by_slug('audiencelab'))),
    true,
